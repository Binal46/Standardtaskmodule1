...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
        ...
        public async Task<IActionResult> GetLanguages(String id = "")
        {
            ...
            User talenUser = null;
            talenUser = await _userRepository.GetByIdAsync(talentId);
            try
            {
                var languages = talenUser.Languages.Select(x => new AddLanguageViewModel
        ...
        public async Task<ActionResult> DeleteLanguage([FromBody] AddLanguageViewModel language)
        {
            try
            {
                String id = "";
                String talentId = String.IsNullOrWhiteSpace(id) ? _userAppContext.CurrentUserId : id;
                User talentUser = null;
                talentUser = await _userRepository.GetByIdAsync(talentId);

                var langugeElement = talentUser.Languages.SingleOrDefault(x => x.Id == language.Id);
                {
                    talentUser.Languages.Remove(langugeElement);
        ...
        public async Task<IActionResult> GetSkills(String id = "")
        {
            ...
            try
            {
                var skills = talenUser.Skills.Select(x => new AddSkillViewModel 
        ...
        public async Task<IActionResult> DeleteSkill([FromBody]AddSkillViewModel skill)
        {
            try
            {
                ...
                var skillElement = talentUser.Skills.SingleOrDefault(x => x.Id == skill.Id);
                {
                    talentUser.Skills.Remove(skillElement);
                    await _userRepository.Update(talentUser);
                    return Json(new { Success = true });
                }
            }
        ...
        public async Task<IActionResult> getExperience(String id = "")
        {
            ...
            try
            {
                var experience = talenUser.Experience.Select(x => new ExperienceViewModel
                { Id = x.Id, Company = x.Company, Position = x.Position,
                Responsibilities = x.Responsibilities, Start = x.Start, End = x.End}).ToList();
        ...
        public async Task<IActionResult> DeleteExperience([FromBody] ExperienceViewModel experience)
        {
            try
            {
                ...
                var experienceElement = talentUser.Experience.SingleOrDefault(x => x.Id == experience.Id);
                {
                    talentUser.Experience.Remove(experienceElement);
                    await _userRepository.Update(talentUser);
                    return Json(new { Success = true });
                }
            }
